...
Since the previous version the $\mtautau$ offset has been \emph{localised}, and this
sharpens the decision. It lives in the $\tau$ four-vectors, not in the mass estimator or
the $\ptmiss$; fake $\tauh$ are excluded as its cause by direct measurement; and the whole
of it --- both the offset and an unphysical high-mass tail --- sits in the
\emph{gen-matched} population. Since the energy-scale map is a per-$p_{T}$ \emph{median}
ratio, it can align the peak but cannot reshape a response distribution, so the residual
appears to lie outside what the current correction method can reach
...
decaying ones, whereas the map is derived on the hadronic visible set only. Both are real
defects and worth fixing, but neither explains the $\mtautau$ offset --- see
Section~\ref{sec:mtautau}, where fakes are excluded by measurement.
...
\paragraph{The split, and what it excludes.} That hypothesis is now tested and
\textbf{wrong}. Splitting every distribution into gen-matched and fake components gives
fake fractions of $4.1\%$ on Delphes against $4.3\%$ on CMS, with the two fake shapes
indistinguishable; the fake components are simply not different between the two tiers.
The gen-matched component carries everything --- the peak displacement and the whole
$\mvis>125\,\si{\GeV}$ tail. The energy-scale treatment is therefore failing on
\emph{real} $\tau$. This matches the $\dR_{\tau\tau}$ result of Section~1.3, where the
discrepancy also lived entirely in the gen-matched component; both $\tau$-side
disagreements now point the same way.

\paragraph{Why this is a statement about the representation.} \texttt{tau\_escale} is a
per-$p_{T}$-bin \emph{median} ratio, applied multiplicatively. By construction it can
align medians; it cannot change the \emph{shape} of the response distribution. An $R=0.4$
cone absorbs underlying event and nearby particles for some $\tau$, where an HPS signal
cone would not, which produces a one-sided high tail in the response. Such a tail is
\emph{not reachable} by any multiplicative median map: one can move the peak into place
and still be left with events at $\mvis>125\,\si{\GeV}$, since a scale factor cannot
remove entries from a kinematically forbidden region. If confirmed by the response-shape
measurement below, this is a hard floor on the correction approach rather than a defect in
its implementation, and it bears directly on Section~\ref{sec:decision}.

\paragraph{The measurement that would confirm it.} Plot the $\tau$ energy response ---
reco $\tau$ $p_{T}$ over gen-visible-$\tau$ $p_{T}$ --- as a \emph{distribution} on both
sides rather than as a per-bin median: the same move $\mvis$ made for $\mtautau$, looking
at what the map is built from instead of what it produces. A longer Delphes high tail at
matched median confirms the paragraph above. Agreeing shapes would instead indict the
derivation, with selection bias the prime suspect, since the response is measured only on
$\tau$ that were both reconstructed and passed ID.
...
\emph{(ii) A symmetric $\tau$ energy resolution mismatch.} Widening the response from
$7\%$ to $25\%$ at fixed median moves the mean by $0.05\%$; it does broaden the
$\mtautau$ RMS by $\sim2\%$, which is worth correcting for shape, but it cannot shift the
peak. Note the scope of this one: it was measured with a roughly \emph{symmetric}
(log-normal) widening, so it does \emph{not} cover the one-sided high tail discussed
above, which is a different object. Of the candidates tested, only the energy
\emph{scale} --- which enters $\mtautau$ linearly --- and a one-sided response tail remain
capable of what is observed.
...
\paragraph{The strongest argument, if the response measurement confirms it.} The
result of Section~\ref{sec:mtautau} changes the character of this decision. The residual
$\mtautau$ disagreement is not a mis-measured number: it sits in gen-matched $\tau$, and
the correction that would have to remove it is a multiplicative per-$p_{T}$ median, which
cannot reshape a distribution. If the $\tau$ energy response really is one-sidedly
broader on Delphes --- as an $R=0.4$ cone absorbing underlying event would make it --- then
\emph{no} refinement of the current maps can close the gap, because a scale factor cannot
depopulate a kinematically forbidden region. That is a hard floor on the correction
approach, and it is the one argument here that a referee could not answer with ``tune it
better''.

Note also what the audit did \emph{not} support. The claim that rebuilding retires three
corrections at once is true but badly weighted: the visible-mass map's residual
imperfection is worth $0.11\%$ on $\langle\mtautau\rangle$ (Section~\ref{sec:mtautau}), so
retiring it buys almost nothing --- the map already does the work. The visible-$\tau$
reference is a derivation-side simplification, not a per-event gain. Effectively one of
the three matters, and it is the energy scale.
...
\item \textbf{Gen-matched / fake split}
(\texttt{scripts/nsbi\_overlay.py --split-gen-matched --diagnostics}). Each side is split
into pairs whose two $\tau$ candidates both sit on a hadronic gen $\tau$ and those that do
not, with both components normalised to the side's \emph{total} so the area under the fake
curve is the fake fraction. Applied to $\mvis$ it excludes fakes as the cause of the
$\mtautau$ offset: $4.1\%$ Delphes against $4.3\%$ CMS with indistinguishable shapes,
while the gen-matched component carries the whole discrepancy. The same tool gave the
corresponding $\dR_{\tau\tau}$ result quoted in Section~1.3.
