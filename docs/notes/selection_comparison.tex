...
Table~1 and Sections~5--6) against the two selections implemented in the Delphes
ntuple converter: a deliberately loose \emph{preselection} (the default) and
\texttt{--cms-selection}. The purpose is to make every difference explicit, so that
yields and sensitivities computed on the Delphes samples are read with the right
caveats rather than compared naively against the published numbers.
\end{abstract}

\section{Scope}

The converter (\texttt{scripts/delphes\_to\_sbi.py}) turns merged Delphes ntuples into
the flat SBI input format. It has two selection modes serving different purposes:

\begin{description}\itemsep2pt
\item[Preselection (default).] One light lepton, one $\tauh$, two jets. Built to mirror
  the \emph{inputs of the NSBI test}, not the CMS analysis. It applies no b-tag
  requirement, no charge requirement, no vetoes and no mass windows. This is why its
  signal$/t\bar t$ acceptance ratio is $\sim$2.5 rather than the hundreds a real
  $b\bar b\tau\tau$ selection gives.
\item[\texttt{--cms-selection}.] The HIG-25-008 resolved selection, as far as Delphes
  can support it. Intended for a comparison against the published expected results.
\end{description}

Both are semi-leptonic by default ($\tau_\mu\tauh$, $\tau_e\tauh$);
\texttt{--channels mt,et,tt} adds $\tauh\tauh$.

\section{Event content and channels}

\begin{table}[htbp]
\centering\small
\begin{tabular}{@{}lp{4.3cm}p{3.4cm}p{4.0cm}@{}}
\toprule
& \textbf{CMS HIG-25-008} & \textbf{preselection} & \textbf{\texttt{--cms-selection}} \\
\midrule
Channels & $\tau_\mu\tauh$, $\tau_e\tauh$, $\tauh\tauh$, boosted-$\tauh\tauh$
         & $\ell\tauh$ only & $\tau_\mu\tauh$, $\tau_e\tauh$ \ok;
           $\tauh\tauh$ opt-in \\
Channel assignment & $\mu \rightarrow \tau_\mu\tauh$, else $e \rightarrow \tau_e\tauh$,
                     else 2nd $\tauh \rightarrow \tauh\tauh$ (\S5)
                   & none & same priority \ok \\
...
\begin{table}[htbp]
\centering\small
\begin{tabular}{@{}lp{4.6cm}p{3.1cm}p{3.9cm}@{}}
\toprule
& \textbf{CMS (Table~1)} & \textbf{preselection} & \textbf{\texttt{--cms-selection}} \\
\midrule
Muon $\pt$ & $>22/26/6\,\si{\GeV}$ (cross $\mu$-$\tau$ / single $\mu$ / VBF+$\mu$)
           & $>20$ & $>22$ (cross) \ok \\
Muon $|\eta|$ & $<2.4$ & $\le 2.4$ \ok & $\le 2.4$ \ok \\
Muon ID, isolation & Tight Muon ID, PF rel.\ iso $<0.15$ & \no\ none & \no\ none \\
\midrule
Electron $\pt$ & $>25/31/13(18)\,\si{\GeV}$ (cross $e$-$\tau$ / single $e$ / VBF+$e$)
               & $>20$ & $>25$ (cross) \ok \\
Electron $|\eta|$ & $<2.5$ & $\le 2.4$ & $\le 2.5$ \ok \\
...
$\tauh$ $\pt$ & 32 ($\tau_\mu\tauh$), 35 ($\tau_e\tauh$),
              ...
              & $>20$ & 32 / 35 \ok; 40 ($\tauh\tauh$) \ok \\
$\tauh$ $|\eta|$, $|d_z|$ & $<2.5$, $<0.2\,\si{\cm}$ & $\le 2.3$ & $\le 2.5$ \ok, no $d_z$ \\
...
$H\rightarrow b\bar b$ jets & AK4, $\pt>20$, $|\eta|<2.5$,
                            ...
                            & $\pt>20$, $|\eta|\le2.4$, $\Delta R>0.4$
                            & \ok\ exactly as CMS \\
b-jet pair assignment & HH-\textsc{bTag} DNN (\S5) & two highest b-tag
                      & \no\ two highest b-tag \\
...
\begin{tabular}{@{}lp{4.6cm}p{3.1cm}p{3.9cm}@{}}
\toprule
& \textbf{CMS} & \textbf{preselection} & \textbf{\texttt{--cms-selection}} \\
\midrule
Opposite charge & required (Table~1) & \no\ no & \no\ no --- see \S\ref{sec:cannot} \\
$\Delta R(\ell,\tauh)$ & $>0.5$ (resolved); $<0.8$ (boosted) & none & $>0.5$ \ok \\
Additional-lepton veto & required (Table~1, \S5) & \no\ no & yes \ok \\
Signal region & ellipse in $(\mtautau,\mbb)$, Eqs.~2--4 & \no\ none & yes \ok \\
$\mtautau$ used in the ellipse & DNN-regressed $\tau\tau$ mass (\S5) & --- & FastMTT
  (closest available) \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{The default preselection is not the CMS selection} and should never be
  compared directly against published yields. Its purpose is comparability with the NSBI
  test's own inputs.
