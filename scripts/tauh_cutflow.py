...
def _cutflow(ev, *, nano, pt_min=20.0, eta_max=2.3, dr=0.4, jet_pt_min=20.0,
             jet_eta_max=2.4, clean_dr=0.4, mtautau_min=20.0):
    ...
    if nano:
        ...
        vis = gj[matched_to_any(gj, gen_taus, dr)]          # neutrino-filtered = visible
    ...
    return [n_events, int(s1.sum()), int(s2.sum()), int(s3.sum()), int(s4.sum()), int(s5.sum())]
...
def main(argv=None) -> int:
    ...
    for d in sorted(glob.glob(os.path.join(args.delphes_dir, "*kl-*"))):
        ...
        cn = _cutflow(NanoAODEvents(nano_by_kl[kl], branches=branches, wp=wp,
                                    entry_stop=args.max_events),
                      nano=True, mtautau_min=args.mtautau_min)
